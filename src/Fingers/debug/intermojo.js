	...
	fingers.editView.addInstanceMethods({
	    ...
	    requestUpdateData: function (msg) {
	        ...
	        if (this.isMyMessage(msg)) {
	            ...
	            if (msg.element) {
	                for (var i = 0; i < msg.element.length; i++) {

	                    fingers.formData.element(that.uid).find('input[type=' + msg.element[i].attr('type') + ']').first().replaceWith(msg.element[i]);
...
*/
	...
	fingers.util.message.addInstanceMethods({
	    ...
	    init: function (recipientId, value, element) {
...
(function (fingers) {
    ...
    fingers.middleware.addStaticMethods({
        ...
        urlOrFileUpload: fingers.middleware.createChild('urlorfileupload-middleware')

    });

    fingers.middleware.urlOrFileUpload.addInstanceMethods({

        showStaticView: function () {

            var value = fingers.formData.element(this.uid).find('input[type=text]').first().val();
        ...
        showEditView: function () {
            ...
            fingers.formData.element(this.uid).after(this.editView);
            this.editView.find('input[type=text]').first().val(fingers.formData.element(this.uid).find('input[type=text]').first().val());
        ...
        createEditView: function () {
            ...
            var ele = '<span class="edit-view-control single-text url-or-file-upload">';
            ele += '<input type="radio" name="fileOrUrl" value="text" checked="checked" />';
            ele += '<input type="text" /> <input type="radio" name="fileOrUrl" value="file" /><input type="file" disabled="disabled" />';
            ele += '</span>';
            this.editView = $(ele);
        ...
        persistChange: function () {
            ...
            if (this.editView.find('input[type=radio]:checked').val() === 'file') {
                // need to check if a file has actually been selected yet -- if not, don't do anything
                this.editView.find('input[type=text]').first().val('manually updated file');
            }
            this.okay(null, [this.editView.find('input[type=file]').first().clone(), this.editView.find('input[type=text]').first().clone() ]);

            return this;

        },

        okay: function (value, element) {

            fingers.broker.publish('request update data', fingers.util.message(this.uid, value, element));
        ...
        checkForClick: function (event) {
            ...
            var that = event.data.that;
            var isExitClick = true;


            if ($(event.target).is('span', that.editView)) {
                isExitClick = false;
            }

            that.editView.children().each(function (i) {
                if ($(event.target).is(this.tagName, this)) {
                    isExitClick = false;
                }
            });
        ...
        beginEdit: function () {
            ...
            if (fingers.formData.element(this.uid)) {
                ...
                this.editView.find('input[type=radio]')
                    .bind('click', function (e) {
                        var whatHasBeenClicked = $(e.currentTarget).val();
                        if (whatHasBeenClicked === 'text') {
                            $(that.editView.find('input[type=text]').first()).removeAttr('disabled');
                            $(that.editView.find('input[type=file]').first()).attr('disabled', 'disabled');
                        }
                        else {
                            $(that.editView.find('input[type=text]').first()).attr('disabled', 'disabled');
                            $(that.editView.find('input[type=file]').first()).removeAttr('disabled');
                        }
                    });
